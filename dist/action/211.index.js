...
/* harmony import */ var _reporting_output_js__WEBPACK_IMPORTED_MODULE_22__ = __webpack_require__(74577);
 ...
 */
async function finalizeWorkflow(octokit, context, previousReviewInfo, coreCheckId, results, reports, findingObservations, shouldFailAction, failureReasons, canResolveStale, gate, triggerErrors, skippedTriggers, inputs, service, memoryRecall) {
...
async function runAnalyzeMode(inputs, initResult, span) {
    const { context, runnerConcurrency, matchedTriggers, skippedTriggers, skipCoreCheck, memoryRecall, } = initResult;
...
async function runReportMode(octokit, inputs, initResult, repoPath, span) {
    const { context, service, auxiliaryOptions, matchedTriggers, skippedTriggers, skipCoreCheck, } = initResult;
    ...
    try {
        ...
        await finalizeReportWorkflow(octokit, context, previousReviewInfo, results, reviewPhase.reports, reviewPhase.findingObservations, reviewPhase.shouldFailAction, reviewPhase.failureReasons, canResolveStale, gate, triggerErrors, { failOnWriteError: true, skippedTriggers, inputs, service, ...replayMemoryOptions });
    }
    catch (error) {
...
async function runPRWorkflow(octokit, inputs, eventName, eventPath, repoPath) {
    ...
    return _sentry_js__WEBPACK_IMPORTED_MODULE_2__/* .Sentry.startSpan */ .sQ.startSpan({ op: 'workflow.run', name: 'review pull_request' }, async (span) => {
        ...
        const { context, service, runnerConcurrency, auxiliaryOptions, matchedTriggers, skippedTriggers, skipCoreCheck, memoryRecall, } = initResult;
        ...
        if (skipCoreCheck) {
            ...
            const findingsOptions = (0,_reporting_output_js__WEBPACK_IMPORTED_MODULE_22__/* .buildBaseOutputOptions */ .vM)(inputs, toSkippedTriggers(skippedTriggers, context));
            try {
        ...
        await finalizeWorkflow(octokit, context, previousReviewInfo, coreCheckId, results, reviewPhase.reports, reviewPhase.findingObservations, reviewPhase.shouldFailAction, reviewPhase.failureReasons, canResolveStale, gate, triggerErrors, skippedTriggers, inputs, service, memoryRecall);
        (0,_base_js__WEBPACK_IMPORTED_MODULE_20__/* .handleTriggerErrors */ .a3)(triggerErrors, matchedTriggers.length);
    });
}
...
const FindingFeedItemSchema = schemas/* object */.Ik({
    ...
    observedAt: api_TimestampSchema.nullable(),
    completedAt: api_TimestampSchema,
}).strict();
...
const CostAggregateResponseSchema = schemas/* object */.Ik({
    groups: schemas/* array */.YO(schemas/* object */.Ik({
