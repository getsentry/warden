...
export async function runPRWorkflow(octokit, inputs, eventName, eventPath, repoPath) {
    return Sentry.startSpan({ op: 'action.pr', name: 'warden.action' }, async (span) => {
        span.setAttribute('github.event', eventName);
        const { context, config, matchedTriggers } = await Sentry.startSpan({ op: 'workflow.phase', name: 'workflow.init' }, () => initializeWorkflow(octokit, inputs, eventName, eventPath, repoPath));
        ...
        const { coreCheckId, previousReviewInfo } = await Sentry.startSpan({ op: 'workflow.phase', name: 'workflow.setupGitHubState' }, () => setupGitHubState(octokit, context));
        const results = await Sentry.startSpan({ op: 'workflow.phase', name: 'workflow.executeAllTriggers' }, () => executeAllTriggers(matchedTriggers, octokit, context, config, inputs));
        const reviewPhase = await Sentry.startSpan({ op: 'workflow.phase', name: 'workflow.postReviews' }, () => postReviewsAndTrackFailures(octokit, context, results, inputs));
        ...
        await Sentry.startSpan({ op: 'workflow.phase', name: 'workflow.resolveStale' }, () => evaluateFixesAndResolveStale(octokit, context, reviewPhase.fetchedComments, allFindings, canResolveStale, inputs.anthropicApiKey));
