...
import { SkillDefinitionSchema, ToolNameSchema } from '../config/schema.js';
 ...
 */
export async function loadSkillFromMarkdown(filePath) {
    let content;
    try {
 ...
 */
export async function loadSkillFromToml(filePath) {
    let content;
    try {
        content = await readFile(filePath, 'utf-8');
    }
    ...
    if (!validated.success) {
        throw new SkillLoaderError(`Invalid skill definition in ${filePath}: ${validated.error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join(', ')}`);
    }
    return {
        ...validated.data,
        rootDir: dirname(filePath),
 ...
 */
export async function getBuiltinSkill(name) {
 ...
 */
export async function getBuiltinSkillNames() {
    const skillsDir = getBuiltinSkillsDir();
 ...
 */
export async function discoverAllSkills(repoRoot) {
 ...
 * 4. Built-in skills
 */
export async function resolveSkillAsync(nameOrPath, repoRoot, inlineSkills) {
