# Wrdn Parent Skill Writer Specification

## Intent

The `wrdn-parent-skill-writer` skill is the repo-owned authoring skill for broad parent skills in Warden.

It exists to help create the "fat" top-level skill that sets direction, coverage, and boundaries for downstream focused skills and future coordinator synthesis.

## Scope

In scope:

- Creating or updating broad Warden parent skills under `skills/`.
- Writing parent-skill `SKILL.md`, `SPEC.md`, `SOURCES.md`, and focused references.
- Encoding concern boundaries, coverage dimensions, and evidence expectations for downstream decomposition.
- Applying upstream skill-writer techniques to Warden's coordinated-skill use case.

Out of scope:

- Writing complete focused child skills end-to-end; use `wrdn-skill-writer`.
- Implementing the runtime coordinator in code.
- Designing generic skill-authoring workflows unrelated to Warden.

## Users And Trigger Context

- Primary users: coding agents working on broad Warden review skills and coordinator-mode design.
- Common user requests: "write the fat parent skill", "make this broad skill decomposable", "create a parent skill", "create a meta skill", "add direction and coverage to this top-level Warden skill".
- Should not trigger for: focused child skill creation, normal code changes, or generic prompt writing.

## Runtime Contract

- Required first actions:
  - verify the task is parent-skill authoring
...
- Non-negotiable constraints:
  - do not turn the parent skill into a generic catch-all review
  - do not silently author child skills as part of the parent-skill pass
...
- Issue #274 design intent for coordinator mode
...
- commit logs/changelogs: changes to coordinated-skill design and bundled-skill packaging
- issue or PR feedback: reports that a broad skill is too vague, too broad, or cannot drive task decomposition
- eval results: future coordinator-mode authoring and synthesis evals
...
- `SKILL.md` contains routing, parent-skill authoring workflow, validation gates, and handoff constraints.
...
- `references/parent-skill-contract.md` contains the contract for a broad parent skill.
- `references/transformed-examples.md` contains good and bad parent-skill patterns.
...
- Lightweight validation:
  ...
  - verify transformed examples remain concrete and parent-skill specific
- Deeper evaluation:
  - exercise prompts for creating and refining broad coordinated skills
...
- Acceptance gates:
  ...
  - the parent-skill contract is explicit about concern boundaries and exclusions
  - transformed examples remain concrete
  - bundled-skill guidance matches current Warden packaging behavior

## Known Limitations

- The skill helps author coordinated parent skills, but the runtime coordinator code path still needs separate implementation.
...
- Update `SKILL.md` when routing, trigger language, or parent-skill workflow changes.
- Update `SOURCES.md` when the source inventory, decisions, or gaps change.
- Update `references/parent-skill-contract.md` whenever Warden's coordinated parent-skill expectations change.
- Update `references/transformed-examples.md` when repeated good or bad parent-skill patterns emerge.
