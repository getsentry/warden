# Wrdn Superwarden Skill Writer Sources
...
| Issue `#274` Superwarden design | product intent | high | Primary source for Superwarden terminology and goals. |
| `specs/superwarden.md` | product spec | high | Canonical repo definition of Superwarden layout, terms, and runtime status. |
...
| Superwarden skill contract | covered | `references/parent-skill-contract.md` defines what a broad Superwarden skill must contain. |
| Superwarden examples | covered | `references/transformed-examples.md` includes happy-path, robust, and anti-pattern examples. |
...
- Separate Superwarden authoring from full child-skill authoring.
- Ship the parent-skill writer in `skills/` so it installs with Warden's bundled skills.
- Keep the parent-skill writer focused on broad direction, coverage, and boundaries, not Superwarden runtime code.

## Open Gaps

- Wire Superwarden runtime code to consume Superwarden artifacts cleanly.
- Add evals once Superwarden exists end-to-end.
- Add repo-owned validation tooling if upstream validator drift becomes painful.

## Changelog

- 2026-04-29: Added `wrdn-parent-skill-writer` as a bundled Warden-owned skill for broad Superwarden authoring.
