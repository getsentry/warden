...
export BENCH_ROOT="/tmp/warden-sentry-benchmark-${BENCH_MODEL_SLUG}"
mkdir -p "$BENCH_ROOT"
```
...
for target in "$BENCH_ROOT"/targets-*.txt; do
  ...
  pnpm cli -- run \
    ...
    --config "$BENCH_ROOT/warden.toml" \
    --model "$BENCH_MODEL" \
    --report-on low \
    --min-confidence low \
    --parallel 4 \
    -o "$BENCH_ROOT/sentry-security-review-pi-${BENCH_MODEL_SLUG}-corpus-${short}.jsonl" \
    -v \
    --log
done
```
...
```bash
pnpm cli -- runs show "$BENCH_ROOT"/sentry-security-review-pi-"$BENCH_MODEL_SLUG"-corpus-*.jsonl \
  -C /tmp/sentry-benchmark \
  --report-on low \
  --min-confidence low
```
