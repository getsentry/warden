...
The `ingest` role can submit runs, write inspect reviews, and request server-side memory extraction. It cannot read findings, history, costs, or memory. Memory recall requires `read`; only combine `--role ingest --role read` on a repository-scoped token. Do not distribute a tenant-wide read token through organization-level Actions secrets.
...
Open **API access** in the dashboard and create a personal token. The token is shown once, expires after 90 days, and accepts only `GET` and `HEAD` read requests. It cannot ingest runs, write inspect reviews, call memory recall, change memory, administer retention, delete data, or manage tokens. The dashboard finding page shows the current review but has no write UI.
...
Useful read routes include `/api/v1/findings`, `/api/v1/runs`, `/api/v1/repositories`, `/api/v1/skills`, `/api/v1/costs`, `/api/v1/outcomes/summary`, and `/api/v1/export`. Filters use the same query parameters as the Explore dashboard. Finding items include the latest observation `outcome` and, when present, a current inspect `review` (`verdict`, `comment`, `updatedAt`). The review does not replace `outcome`. Filter labelled findings with `review=false_positive`, `review=true_positive`, or `review=mitigated`.
...
| Variable | Purpose |
| --- | --- |
| `WARDEN_SERVICE_URL` | Sets the credential-bearing service endpoint |
...
Replay uses the artifact's original run ID and the current profile validation and redaction rules. Sending the same artifact again is idempotent. Warden reads the file without changing or deleting it. Replay never runs automatically. If `.warden/reviews/<runId>.json` exists, replay publishes the run first, then pushes those inspect labels. Inspect labels are a later event on an already-published run: publish the run before reviews can apply. An ingest token writes the reviews; a missing run returns 404 and does not create one.
