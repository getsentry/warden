...
files, review whole files, or merge nearby hunks for more context. The global
scan policy runs before chunking; use `defaults.ignore.paths` to re-include
built-in ignored files before assigning them a chunking mode.
