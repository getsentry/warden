...
## Offline model catalogs

When `runtime = "pi"`, Warden refreshes the selected provider catalog before
model resolution so newly published models are available without a package bump.
Disable that network refresh in durable config:
...
`defaults.offline` is preferred for permanent offline policy. It blocks both
remote skill network fetches and Pi model-catalog refresh. CLI `--offline` or
`WARDEN_OFFLINE` does the same for one run. `PI_OFFLINE` is a Pi-catalog-only
override and does not force remote skills onto cache-only mode.
...
| `moonshotai/kimi-k3` | `WARDEN_MOONSHOT_API_KEY` |
| `openrouter/moonshotai/kimi-k3` | `WARDEN_OPENROUTER_API_KEY` |
...
across runtimes. Supported values are `off`, `low`, `medium`, `high`, `xhigh`,
and `max`. When omitted, Warden sends explicit `high` adaptive thinking to the
Claude runtime; Pi uses its own default thinking level.

`defaults.auxiliary.effort` optionally controls reasoning effort for Pi-backed
auxiliary structured calls such as extraction repair, verification, merge,
dedupe, and fix evaluation. When omitted, Pi uses its own default. Claude
structured helper calls currently ignore this field.
...
`defaults.auxiliary.effort` is independent of `--effort` and only affects
the auxiliary lane.
