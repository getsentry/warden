...
remote skill network fetches and Pi model-catalog refresh. CLI `--offline`
does the same for one run. `PI_OFFLINE` is a Pi-catalog-only override and does
not force remote skills onto cache-only mode.
