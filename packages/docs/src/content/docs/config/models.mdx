...
`defaults.offline` is preferred for permanent offline policy. It also keeps
remote skill loading on cache-only behavior. For one-off runs, use CLI
`--offline` or set `PI_OFFLINE` in the process environment. Any of these blocks
Pi model-catalog network access.
