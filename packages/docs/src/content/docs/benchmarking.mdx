...
The DeepSeek V4 Pro row uses Pi through OpenRouter with explicit `--effort
xhigh`. It scans the same 156 chunks with zero failed chunks after targeted
repairs, uses Warden's finding verifier, and has agent-verified scoring.
DeepSeek found 23 of 86 known corpus entries and emitted 30 total findings.

That ties Opus 4.6 high on known matches, beats Opus 4.8 high on Pi by two
known matches, and trails Sonnet 4.6 on Pi by two. It does not reach GPT 5.5
high, which found 41 of 86. Six emitted DeepSeek findings did not match the
corpus, and one cell-service issue was reported twice from adjacent chunks but
only counted once for known-corpus coverage.

The operational shape is different from the Claude rows. DeepSeek used 3,019
turns and 3,502 tool executions across the corpus: much more tool work than
Opus 4.6 high's 981 turns and 1,101 tool executions. It also consumed 62.2M
scan input tokens. Recorded scan cost was still only $9.76, with $18.70 total
including verifier and repair work, because the OpenRouter/DeepSeek token price
is low.

The cost win comes with a reliability and latency warning. The 7f41cc50 and
9fc06de5 shards needed targeted repair reruns after provider and extraction
failures. Those repair records are traced and included in the committed
aggregate cost; abandoned failed attempts are excluded. P50 chunk duration was
3.8 minutes and P90 was 19.3 minutes, so this row is not pleasant to operate
even though the provider bill is low.
...
with explicit xhigh effort. Opus 4.7 on Pi found 6 of 86 at Pi's default level.
