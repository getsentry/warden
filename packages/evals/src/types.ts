...
export const EvalScenarioNotesSchema = z.object({
  ...
  /** Mapping from checked-in fixture paths to source repository paths. */
...
export const EvalScenarioSchema = z.object({
  ...
  given: z.string(),
  /** Fixture files to use, relative to the eval package root */
  files: z.array(z.string()).min(1),
  /** Supporting fixture files copied into the eval repo without being part of the diff */
...
export interface EvalMeta {
  ...
  filePaths: string[];
  /** Resolved absolute paths to supporting files copied before the eval diff */
