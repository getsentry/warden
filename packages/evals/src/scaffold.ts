...
async function fetchLicenseFileContent(
  octokit: Octokit,
  pull: GitHubPullRequestRef,
  ref: string,
): Promise<(GitHubFileContent & { sourcePath: string }) | undefined> {
...
function scenarioJson(args: {
  ...
  return `${JSON.stringify({
    given: args.title,
    files: args.files.map((file) => file.fixturePath),
    supporting_files: args.supportingFiles,
    should_find: [{
    ...
    notes: {
      ...
      source_files: args.files.map((file) => ({
...
export async function scaffoldEvalFromGitHubPullRequest(
  ...
  const supportingFiles: ScaffoldedEvalFile[] = [];
  const skippedFiles: SkippedScaffoldFile[] = [];
  const contents: (ScaffoldedEvalFile & { content: string })[] = [];
  const supportingContents: (ScaffoldedEvalFile & { content: string })[] = [];
  ...
  const license = await fetchLicenseFileContent(octokit, pull, ref);
  if (!license) {
    throw new Error(
      `No root LICENSE file could be scaffolded from ${pull.owner}/${pull.repo} at ${side} ref ${ref}`
    );
  }

  if (!copiedFiles.some((file) => file.sourcePath === license.sourcePath)) {
    const fixturePath = posix.join(
      'fixtures',
      name,
      fixturePathForSource(pull, license.sourcePath, seenFixturePaths),
    );
    const fullFixturePath = fromEvalsPath(options.evalsDir, fixturePath);
    if (!options.force && existsSync(fullFixturePath)) {
      throw new Error(`Eval fixture already exists: ${fullFixturePath}`);
  ...
  for (const content of [...contents, ...supportingContents]) {
  ...
  writeFileSync(
    ...
    scenarioJson({
      ...
      body: pr.body,
      files: copiedFiles,
      supportingFiles: supportingFiles.map((file) => file.fixturePath),
      url: options.url,
