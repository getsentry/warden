...
/** Return the repo-local root that stores Superwarden parent skills and task artifacts. */
...
instructions:
  ...
  - Preserve the context that justified the task split in the parent plan and record unresolved context explicitly instead of inventing facts.
`, 'utf-8');
...
- The parent plan is a lean decomposition artifact, not a mini child skill.
...
- The parent plan must record the scope kind, observed context, unresolved context, and lean task blueprints that justify child synthesis.
...
- Lightweight validation: run \`warden synth <name> --show-plan\` and inspect task coverage, scope context, and source provenance.
...
- Superwarden synthesis cannot ask live follow-up questions yet; unresolved context must be recorded in plan or child skill artifacts.
...
- Keep generated task skills as children of the repo-local Superwarden skill, not in \`.agents/skills\`.
...
- Store child synthesis metadata in the parent plan record so matching task skills can be reused without adding extra files to runnable child skill directories.
- Record unresolved synthesis context in the parent plan or child artifacts until Superwarden has a dedicated user-inquiry tool.
...
| Runtime isolation | complete | Generated artifacts live under \`.warden/superwarden/<name>/tasks/\`, not \`.agents/skills\`. |
| Cache reproducibility | complete | The parent plan record stores task/source hashes and child generation telemetry. |
| Missing context handling | partial | Unresolved context is recorded in plan or child artifacts until live user inquiry tools exist. |
| API surface | complete | Superwarden exposes synthesis and run behavior through the Warden CLI and library exports. |
| Config/runtime options | complete | \`mode = "coordinator"\`, model settings, regeneration, and plan paths are part of the runtime contract. |
