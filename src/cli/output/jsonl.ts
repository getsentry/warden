...
/**
 * Render skill reports to a JSONL string (for stdout output).
 * Same format as writeJsonlReport but returns a string instead of writing to a file.
 */
export function renderJsonlReport(
  reports: SkillReport[],
  durationMs: number,
  options?: { runId?: string; traceId?: string }
): string {
